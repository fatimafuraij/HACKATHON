...
$errors = array( ); // Initialize an error array.
   ...
   if (empty( $_POST['NPID'])) {
      $errors = array( ); // Initialize an error array.
           ...
           $errors[] = 'You forgot to enter Product_ID.';
      } else {
          $NPID= trim( $_POST['NPID']);
      }


    
    if (empty( $_POST['Pname'])) {
          $errors[] = 'You forgot to enter Product_Name.';
     } else {
 ...
 // Check for an email address:
       if (empty($_POST['Ptype'])) {
           $errors[] = 'You forgot to Product_Type.';
      } else {
        ...
        if (empty($_POST['Pprice'])) {
           $errors[] = 'You forgot to enter Product_price.';
      } else {
          $Pprice = trim($_POST['Pprice']);
       }
  if (empty($_POST['Plink'])) {
           $errors[] = 'You forgot to enter Product_Image.';
      } else {
...
$q ="INSERT INTO  product (Product_ID, Product_price,  Product_Name, Product_Type,Product_Image ) VALUES ('$NPID', '$Pprice', '$Pname' ,' $Ptype','$Plink')";
...
$sql="SELECT * FROM product WHERE (Product_ID=' $NPID'  );";
